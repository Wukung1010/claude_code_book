...
这条输入历史链里，`expandPastedTextRefs()` 很能说明问题：
...
也就是说，history.ts 保存的不是一串平面的 prompt 文本，而是一套可以把“历史输入中的引用占位符”重新展开的表示法。这就是它为什么更接近“输入体验工程”，而不是“对话 transcript”。
...
配合 `isTranscriptMessage()` 一起看，`isChainParticipant()` 更能说明恢复链的严格边界：
...
这两段代码把 sessionStorage.ts 的设计原则讲得很清楚：并不是所有显示给用户看的事件都能写进 transcript，也不是所有消息都能参与 parentUuid chain。progress 被明确排除在正式恢复链之外，正是为了避免 UI 态污染会话语义。
...
这层抽象在 `deserializeMessagesWithInterruptDetection()` 里体现得尤其明显。它的核心骨架大致是：
...
这段代码很值得反复看，因为它把恢复层的职责说得非常透彻：先清洗历史脏轨迹，再判断中断发生在什么位置，最后把“半轮中断”改写成统一运行时能继续处理的一条 meta user message。resume 的关键不是把旧文件读回来，而是把历史重新规整成当前主循环还认得的输入。
...
这件事在 `restoreSessionStateFromLog()` 和 `restoreWorktreeForResume()` 里都写得很明确：
...
这说明 `sessionRestore.ts` 恢复的并不是单一的 messages 数组，而是把 file history、context collapse、todo 状态、agent 设定、worktree 现场一起重新挂回运行时。也正因为如此，第 9 章讨论的从来不只是 transcript，而是完整的恢复工程层。
...
第 9 章最重要的收获，不是“Claude Code 支持 resume”，而是它把长期可用性拆成了多条彼此独立又能互相接上的工程链。

1. memory/CLAUDE.md/memdir 解决的是长期知识如何进入上下文。
2. history.ts 解决的是交互输入如何稳定回放。
3. sessionStorage、conversationRecovery、sessionRestore 解决的是会话和运行副状态如何被重新装回现场。

所以第 9 章真正讲清楚的是：长期协作 Agent 要能“记得住、接得上、恢复得回来”，靠的从来不是单一 transcript 文件，而是一整套持久化与恢复工程。

## 9.23 这一章和后续章节怎么衔接

第 9 章在全书里的作用很像一条横切基础设施。

1. 它承接第 4 章，因为 memory 和 CLAUDE.md 并不是独立机制，最后都还要重新汇入上下文系统。
2. 它会直接通向第 13 章，因为 continue、resume、teleport 这些恢复路径，本质上都是在这里讲到的 transcript 清洗和状态恢复之上继续分叉。
3. 它也会回流到第 14 章，因为 worktree 恢复、remote 现场映射和会话现场切换，后面都要继续建立在这里的恢复工程层之上。

所以第 9 章不是一本书中途插进来的“工程杂项”，而是统一运行时得以长期存在的持久化底座。
