...
> 对应源码主线：src/entrypoints/cli.tsx、src/main.tsx、src/setup.ts、src/replLauncher.tsx、src/screens/REPL.tsx、src/QueryEngine.ts、src/query.ts
...
为了和后文保持术语一致，这一章里提到的“宿主程序”“运行时装配层”“终端 Agent 操作系统”，都可以统一理解为 Claude Code 的“统一运行时”。
...
1. 第 4 章会展开这里提到的上下文系统，解释统一运行时为什么不是只拿当前用户输入，而是先拼 system context 和 user context。
2. 第 5 章会把这里的“会话编排层”和“单轮循环层”真正拆开，正式确立 `QueryEngine` 与 `query.ts` 的边界。
3. 第 7 章会沿着这里的“能力边界”继续下钻，说明工具系统为什么先定义能力边界，再允许模型行动。
4. 第 10 章会把这里的 REPL 交互层具体化，解释统一运行时在终端 UI 中到底如何托管输入、队列、中断和外部输入来源。
5. 第 14 章会把这里的整体架构进一步扩展到 worktree、remote、ssh、direct connect，说明统一运行时并不绑定在单一本地现场上。

换句话说，第 1 章不是只讲“启动流程”，它其实是前半本所有主线章节的总入口。

## 1.15 这一章的阅读结论

第 1 章最重要的作用，不是把所有细节一次讲完，而是先给整本书搭出一张稳定坐标系。

1. 入口层、会话编排层、单轮循环层这三层边界必须先分开，否则后面读 main.tsx、QueryEngine.ts、query.ts 时很容易混成一个大文件。
2. context、commands、tools、permissions、memory 这些横切系统，必须从一开始就看成运行时的组成部分，而不是“后面再补的功能模块”。
3. Claude Code 真正复杂的地方不是某个单点功能，而是这些分层如何在同一条运行时总线上被装起来。

所以第 1 章的价值，是先把“这到底是一套怎样的系统”讲清楚，后面的逐章精读才能有稳定参照物。
