...
为了和后文术语保持一致，这一章里出现的“能力边界”“可见工具池”“运行态裁剪”，都属于统一运行时里的能力边界治理。
...
第 7 章和后半本书的关系其实比表面更紧：

1. 它承接第 5 章，因为 QueryEngine 里包装 `canUseTool`、记录 permission denials，前提就是这里已经先定义了可见工具池和能力边界。
2. 它向第 8 章展开，因为 commands、skills、MCP tools 会继续补齐“模型能做什么”的完整能力索引。
3. 它向第 18 章延伸，因为这里的前置 deny、调用审批和执行反馈，到了后面会正式展开成一条权限仲裁链。
4. 它也会一直影响第 20 章，因为 remote control、direct connect、bridge 这些模式最终仍然要复用本章定义的工具可见性与审批语义。

所以第 7 章不是孤立的工具注册表，而是后面权限系统和统一控制面的能力边界起点。
